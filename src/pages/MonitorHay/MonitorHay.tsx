...
const socket: Socket<ServerToClientEvents, object> = io(
  "https://getbankv2.vidieu.net/"
);

const MonitorHay = () => {
  const [hay, setHay] = useState<string[]>([]);
  const [activeTab, setActiveTab] = useState<string>("all");

  useEffect(() => {
    const handleHay = (bank: string) => {
      setHay((prev) => {
    ...
    socket.on("hay88", handleHay);

    return () => {
      socket.off("hay88", handleHay);
    };
  }, []);

  const getTotalCount = () => hay.length;

  const getFilteredData = (): MonitorData[] => {
    const allData: MonitorData[] = [{ data: hay, color: "red", title: "HAY" }];
